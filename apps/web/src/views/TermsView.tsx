...
export const TermsView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'terms' | 'privacy' | 'creator' | 'dmca'>(
    initialTab as any || 'terms'
  );

  useEffect(() => {
    const tab = searchParams.get('tab');
    if (tab && ['terms', 'privacy', 'creator', 'dmca'].includes(tab)) {
      setActiveTab(tab as any);
    }
  }, [searchParams]);

  const handleTabChange = (tab: 'terms' | 'privacy' | 'creator' | 'dmca') => {
    setActiveTab(tab);
    setSearchParams({ tab });
